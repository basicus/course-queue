...
func main() {
	var (
		...
		queue   = flag.String("group", "group1", "Group queue name")
	)
	...
	sub, _ := nc.QueueSubscribe(*subject, *queue, func(msg *nats.Msg) {
		...
		resp := models.MessageReply{
			...
			MessageStatus: 200,
			MessageReply:  "OK from " + nc.ConnectedClusterName() + " group " + *queue + " consumer id " + id,
		}
